...
async function loadTemplateFromCandidates(candidates) {
    for (const candidate of candidates){
        try {
            return await __TURBOPACK__imported__module__$5b$externals$5d2f$node$3a$fs__$5b$external$5d$__$28$node$3a$fs$2c$__cjs$29$__["promises"].readFile(candidate);
        } catch  {
        // Continue searching
        }
    }
...
async function applyLetterhead(sourcePdfBuffer, tune) {
    ...
    const { firstPageTemplateBytes, otherPagesTemplateBytes } = await loadLetterheadTemplates();
    const [firstPageTemplate] = await outDoc.embedPdf(firstPageTemplateBytes, [
        0
    ]);
    const [otherPagesTemplate] = await outDoc.embedPdf(otherPagesTemplateBytes, [
        0
    ]);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0xi~o58._.js.map